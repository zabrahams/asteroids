(function () {
  ...
  Asteroid.spawn = function (pos, game) {
    game.hud.scorePoint();
    asteroid1 = new Asteroid({pos: pos, radius: 5, game: game})
    asteroid2 = new Asteroid({pos: pos, radius: 5, game: game})
  ...
  Asteroid.prototype.collideWith = function (otherObject) {
      if (otherObject.constructor === Asteroids.Ship && otherObject.dying === false) {
        otherObject.lives--;
        otherObject.game.hud.loseLife();
        otherObject.dying = true
        Animations.changeSize({
          obj: otherObject,
          change: 1,
          frames: 30,
            ...
            Animations.changeSize({
              ...
              callback: function () {
                if (otherObject.lives <= 0) {
                  delete otherObject.game.ship;
                } else {
                  otherObject.dying = false;
                  if (otherObject.game.DIM_X/60 < 12) {
                    otherObject.radius = 12;
                  } else {
                    otherObject.radius = otherObject.game.DIM_X/60;
                  }
