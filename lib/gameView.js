(function () {
    ...
    GameView.prototype.setupRestartButton = function () {
      if (this.game.isOver()) {
        ...
        if (this.mouseX >= this.game.DIM_X/2 - 50 + this.canvasEl.offsetLeft &&
            this.mouseX <= this.game.DIM_X/2 + 50 + this.canvasEl.offsetLeft &&
            this.mouseY >= this.game.DIM_Y/2 - 50 + this.canvasEl.offsetTop &&
            this.mouseY <= this.game.DIM_Y/2 + 50 + this.canvasEl.offsetTop) {

          this.game.drawRestartButton(this.ctx,
            this.restartRadius,
            Asteroids.Ship.COLOR,
            Asteroids.Ship.INNER_COLOR);
        } else {
          console.log(this.restartRadius);
    ...
    GameView.prototype.clickButton = function (event) {
      if (this.game.isOver() &&
          event.clientX >= this.game.DIM_X/2 - 50 + this.canvasEl.offsetLeft &&
          event.clientX <= this.game.DIM_X/2 + 50 + this.canvasEl.offsetLeft &&
          event.clientY >= this.game.DIM_Y/2 - 50 + this.canvasEl.offsetTop &&
          event.clientY <= this.game.DIM_Y/2 + 50 + this.canvasEl.offsetTop) {
        this.restartRadius = 1;
